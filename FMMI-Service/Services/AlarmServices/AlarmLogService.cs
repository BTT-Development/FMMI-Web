...
internal class AlarmLogService : BaseService<AlarmLogs>, IAlarmLogService
{
    ...
    public async Task<Result<List<ShowAlarmLogDTO>>> GetAllActiveAlarmlogs()
    {
        ...
        alarmlogs = await _context.AlarmLogs.Include(x => x.Alarmer).ThenInclude(x => x.Device).ThenInclude(x => x.Machine).ThenInclude(x => x.Locations).Where(x => x.NewAlarm == true).MapAlarmLogToDTO().ToListAsync();
